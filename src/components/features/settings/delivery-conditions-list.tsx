...
export default function DeliveryConditionsList() {
  const [items, setItems] = useState<DeliveryCondition[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [editing, setEditing] = useState<DeliveryCondition | null>(null);
  ...
  async function load() {
    try {
      setLoading(true);
      const res = await fetch("/api/settings/delivery-conditions", {
        cache: "no-store",
      });
      ...
      setError("Failed to load delivery conditions");
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  function startEdit(item: DeliveryCondition) {
    setEditing(item);
    setShowForm(true);
  }
  ...
  async function handleDelete(id: string) {
    try {
      const res = await fetch(`/api/settings/delivery-conditions/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                ...
                rows={5}
                placeholder="Markdown-friendly conditions"
              />
            </div>
      ...
      {showForm && editing && (
        <div className="bg-card minimal-shadow minimal-border rounded-md p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">
              {editing.id
                ? "Edit Delivery Condition"
      ...
      <ConfirmDialog
        ...
        title="Delete delivery condition?"
        description="This will remove the delivery condition from your settings. This action cannot be undone."
        confirmLabel="Delete"
        isDestructive
        onConfirm={
