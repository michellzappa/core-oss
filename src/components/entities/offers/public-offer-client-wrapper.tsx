...
// Dynamically import the public offer layout with no SSR to avoid crypto issues
const PublicOfferLayout = dynamic(() => import("./public-offer-layout"), {
  ssr: false,
  loading: () => (
...
interface PublicOfferClientWrapperProps {
  offer: any;
  services: any[];
}

export default function PublicOfferClientWrapper({
  offer,
  services,
}: PublicOfferClientWrapperProps) {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-neutral-100 dark:bg-neutral-900 flex flex-col items-center p-4 sm:p-8">
    ...
    >
      <PublicOfferLayout offer={offer} services={services} />
    </Suspense>
  );
}
